...
%WINDOWS_SIGN_SCRIPT_PATH% "%current_path%\dist" && cd "%current_path%\dist\" && for %i in (*.exe) do (set fName=%i) && ren %fName% %fName:~0,-13%.exe && buildkite-agent.exe artifact upload "*.exe"
GOTO END
